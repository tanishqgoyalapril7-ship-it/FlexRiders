...
| `NEXT_PUBLIC_ADMIN_URL` | Where the footer's **Admin Login** button goes: the Super Riders / Flex Riders admin dashboard, which opens on its login screen. Defaults to `http://localhost:5180` (the dashboard's dev server). |
| `CONTACT_WEBHOOK_URL` | Optional. Each "Get Started" or "Talk to Us" enquiry is POSTed here as JSON (for a CRM, email service or chat webhook). If it isn't set, enquiries are only logged on the server. |
