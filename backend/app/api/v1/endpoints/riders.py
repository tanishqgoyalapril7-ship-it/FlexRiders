...
@router.get("/live-current/profile", response_model=Optional[RiderDetailResponse])
def get_live_current_profile(db: Session = Depends(get_db)):
    """Returns the most recent rider in the database for instant mobile simulator sync"""
    rider = db.query(Rider).order_by(Rider.id.desc()).first()
    if not rider:
        return None
...
@router.get("/live-current/payments", response_model=PaymentMonthlySummary)
def get_live_current_payments(month: Optional[str] = "September 2026", db: Session = Depends(get_db)):
    """Returns actual payments for the current active/latest rider"""
    rider = db.query(Rider).order_by(Rider.id.desc()).first()
    if not rider:
        return PaymentMonthlySummary(
            month_name=month,
    ...
    payments = db.query(Payment).filter(Payment.rider_id == rider.id).order_by(desc(Payment.payment_date)).all()
    earnings = calculate_rider_earnings(db, rider.id)
    response_items = [
        PaymentResponse(
            id=p.id,
...
@router.get("/live-current/notifications", response_model=List[NotificationResponse])
def get_live_current_notifications(db: Session = Depends(get_db)):
    """Returns actual notifications for the current active/latest rider"""
    rider = db.query(Rider).order_by(Rider.id.desc()).first()
    if not rider or not rider.user_id:
        return []
    return db.query(Notification).filter(Notification.user_id == rider.user_id).order_by(desc(Notification.created_at)).all()
