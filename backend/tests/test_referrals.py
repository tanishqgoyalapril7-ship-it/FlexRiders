...
def test_referral_reward_after_first_photo_streak(client, db_session):
    ...
    bad = client.post(f"{API}/auth/register", json={"selfie": SELFIE, "vehicle_category": "CYCLE", "accept_terms": True, "full_name": "Bhavesh Kumar", "mobile_number": b_phone, "password": "riderPass1", "referral_code": "SRNOPE00"})
    assert bad.status_code == 400 and "referral code" in bad.json()["detail"]
    reg = client.post(f"{API}/auth/register", json={"selfie": SELFIE, "vehicle_category": "CYCLE", "accept_terms": True, "full_name": "Bhavesh Kumar", "mobile_number": b_phone, "password": "riderPass1", "referral_code": code.lower()})
